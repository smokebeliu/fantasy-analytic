...
Текущий прогресс: 11 из 21 шагов завершён.
...
Колонка «Приоритет» задаёт порядок для незавершённых шагов по смыслу (см.
раздел [«Приоритизация оставшихся шагов»](#приоритизация-оставшихся-шагов)):
`P0` — фундамент, `P1` — ядро ценности, `P2` — усиление подбора, `P3` —
продвинутая аналитика и эксплуатация. Приоритет ортогонален статусу: он говорит
«что важнее делать», статус — «можно ли уже брать в работу».
...
| 14 | Импорт активного сезона 2026/2027 и разделение сезонов | 2, 4, 5 | `P0` | `READY` |
| 15 | Кросс-сезонный прогноз состава на первый тур | 6, 7, 8, 14 | `P1` | `PLANNED` |
| 16 | Полная статистика в таблице игроков (сортировка по очкам, без фильтра по турам) | 9, 10 | `P1` | `READY` |
| 17 | Раздел «Состав»: поле-схема, переключение вида, group-button фильтр | 8, 10 | `P1` | `READY` |
| 18 | Фиксация игроков и подбор оптимального состава под схему | 8, 9, 17 | `P2` | `PLANNED` |
| 19 | Учёт соперников следующего тура в оптимизации | 8, 15 | `P2` | `PLANNED` |
| 11 | Ручное обновление из frontend | 5, 10 | `P2` | `READY` |
| 20 | Сила команд и AI-коэффициенты | 6, 7, 15 | `P3` | `PLANNED` |
| 12 | Backtesting и сравнение моделей | 2, 7 | `P3` | `PLANNED` |
| 13 | Интеграционная сборка и эксплуатационная готовность | 9, 11, 12 | `P3` | `PLANNED` |
...
flowchart LR
    ...
    S11 --> S13
    S12 --> S13
    S2 --> S14[14 Active season]
    S4 --> S14
    S5 --> S14
    S14 --> S15[15 Cross-season forecast]
    S6 --> S15
    S7 --> S15
    S8 --> S15
    S9 --> S16[16 Player table UX]
    S10 --> S16
    S8 --> S17[17 Squad pitch UI]
    S10 --> S17
    S17 --> S18[18 Locked players]
    S9 --> S18
    S15 --> S19[19 Fixture-aware optimize]
    S8 --> S19
    S15 --> S20[20 Team strength & AI]
    S7 --> S20
```
...
- **Шаг 14 (активный сезон 2026/2027).** Без импорта нового сезона и разделения
...
- **Шаг 15 (кросс-сезонный прогноз).** Прямо реализует основную задачу
  пользователя — состав на 1-й тур из прошлогодних данных с учётом ушедших и
  новых игроков. Зависит от 14.
- **Шаг 16 (UX таблицы игроков).** Быстрый видимый эффект и низкий риск: чинит
  сортировку по очкам и убирает лишний фильтр по турам. Уже `READY`.
- **Шаг 17 (переработка раздела «Состав»).** Ядро UX подбора состава (поле-схема,
  переключение вида, group-button фильтр). Уже `READY`.

P1-шаги 16 и 17 не зависят от 14/15 и могут выполняться параллельно с
фундаментом; 15 стартует сразу после 14.

### P2 — усиление подбора состава

- **Шаг 18 (фиксация игроков).** Явно запрошенная функция; сильно повышает
  практическую пользу подбора. Зависит от нового UI (17).
- **Шаг 19 (учёт соперников).** Улучшает качество оптимизации на очных встречах.
  Зависит от кросс-сезонного прогноза (15).
- **Шаг 11 (ручное обновление из frontend).** Операционное удобство: в идущем
...
- **Шаг 20 (сила команд и AI-коэффициенты).** Наибольшая сложность и
  неопределённость (нужна discovery по котировкам, опциональный AI-слой); даёт
  прирост качества поверх уже работающего подбора.
- **Шаг 12 (backtesting).** Внутренняя валидация модели, не виден пользователю
  напрямую; ценен для доверия к прогнозу, но не блокирует сбор состава.
- **Шаг 13 (интеграционная готовность).** Деплой уже работает; остаётся добить
  операционку (graceful shutdown, structured logs, smoke-test). Финальный
  «полировочный» шаг, зависит от 11 и 12.

### Рекомендованный порядок

`14 → 16 → 17 → 15 → 18 → 19 → 11 → 20 → 12 → 13`

16 и 17 подняты вперёд как готовые к работе быстрые улучшения (их можно вести
параллельно с 14); 15 выполняется сразу после появления активного сезона (14).
...
## Дорожная карта доработок (запрос от 2026-07-21)

Шаги 14–20 добавлены по итогам ревью развёрнутого приложения. Основная цель
блока — помочь собрать состав на **новый** турнир РПЛ 2026/2027, опираясь на
исторические данные завершённого сезона 2025/2026, и довести UX разделов
«Игроки» и «Состав». Каждый шаг по-прежнему самодостаточен для независимого
агента.

### Шаг 14. Импорт активного сезона 2026/2027 и разделение сезонов
...
- Проверить кросс-сезонные ключи: `players.stat_player_id` и `clubs.stat_team_id`
  как основа для связки прошлого и нового сезона (нужно шагу 15).
...
Не входит: кросс-сезонный прогноз (шаг 15).

### Шаг 15. Кросс-сезонный прогноз состава на первый тур
...
- В прогнозе/фиче помечать источник статистики: `prior_season` vs
  `current_season`, чтобы фронт (шаги 16–17) визуально отделял историческую
...
Не входит: учёт соперников и силы команд (шаги 19–20).

### Шаг 16. Полная статистика в таблице игроков
...
- Визуально разделить историческую статистику прошлого сезона и статистику
  текущего сезона (бейдж/группировка), опираясь на источник из шага 15.
...
Не входит: изменения раздела «Состав» (шаг 17).

### Шаг 17. Раздел «Состав»: поле-схема, переключение вида, group-button фильтр
...
- Учесть разделение статистики прошлого/текущего сезона из шага 15 в карточках
...
Не входит: фиксация игроков и подбор под схему (шаг 18).

### Шаг 18. Фиксация игроков и подбор оптимального состава под схему
...
### Шаг 19. Учёт соперников следующего тура в оптимизации
...
Не входит: коэффициенты силы команд и AI (шаг 20).

### Шаг 20. Сила команд и AI-коэффициенты

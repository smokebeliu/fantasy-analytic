...
Завершённые шаги (0–10) идут по порядку выполнения, далее незавершённые шаги
расположены в порядке приоритета (см.
[«Приоритизация оставшихся шагов»](#приоритизация-оставшихся-шагов)):
`14 → 16 → 17 → 15 → 18 → 19 → 11 → 20 → 12 → 13`. Номер шага — устойчивый
идентификатор для независимого агента и не меняется при перестановке, поэтому
нумерация карточек намеренно не последовательна.

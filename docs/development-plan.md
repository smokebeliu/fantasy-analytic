...
- **Двухколоночная раскладка (запрос 6).** `SquadBuilder.tsx` переработан: слева —
  текущий состав в виде схемы на поле (новый компонент `SquadPitch.tsx`,
  переиспользует стили `.pitch`/`.pitch-player` из `OptimizerPitch.tsx`), справа —
  панель фильтра/поиска и компактный список всех игроков (`.pool-list`/`.pool-item`
  вместо широкой таблицы, помещается в узкую колонку и скроллится). Сводка правил
  (метрики, счётчики позиций, нарушения, кнопки) переехала в левую колонку к схеме.
- **Схема состава.** `SquadPitch` раскладывает выбранных игроков по рядам ВРТ→ЗАЩ→
  ПЗЩ→НАП; пустые слоты рисуются по `roleLimits.max` (только когда сумма максимумов
  равна размеру ростера — иначе fallback-лимиты нарисовали бы бессмысленное число
  слотов). Клик по пустому слоту фокусирует список на нужной позиции (устанавливает
  фильтр), карточка игрока на поле имеет кнопку удаления (×), видимую при наведении.
- **Переключатель вида (запрос 6).** Group-button `Схема ↔ Список` в шапке левой
  колонки; список — прежний `selected-list` с бейджами ролей и удалением.
- **Group-button фильтр по позиции (запрос 5).** `<select>` заменён на сегментный
  переключатель `Все / ВРТ / ЗАЩ / ПЗЩ / НАП` (класс `.role-group`, `aria-pressed`),
  который меняет серверный фильтр пула. Тот же стиль используется для переключателя
  вида.
- **Согласованность.** Добавление из списка и удаление из схемы/списка идут через
  общий `selected`-state, поэтому метрики, счётчики позиций и валидация (`lib/squad`)
  остаются синхронными; кнопки добавления по-прежнему блокируются `canAddPlayer`, а
  «Оптимизировать трансферы» — до валидного состава из 15 игроков. «Автосостав» и
  результат оптимизатора не менялись.
- **Тесты.** Vitest: `SquadPitch.test.tsx` (3 сценария: рендер игроков и пустых
  слотов, удаление + клик по слоту, подавление слотов при нестандартных лимитах) и
  расширенный `SquadBuilder.test.tsx` (group-button фильтр вызывает рефетч с ролью,
  переключение схема↔список). Playwright: два новых e2e — раскладка со схемой +
  group-button фильтр (пул только защитников) и переключение вида схема↔список.
  Всего Vitest 35, Playwright 10 — зелёные против живого backend.

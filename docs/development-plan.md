...
Текущий прогресс: 20 из 22 шагов завершён.
...
Шаги 18 и 20 остаются незавершёнными; шаг 21 добавлен после них по номеру, но
относится к тиру `P1` — номер здесь только устойчивый идентификатор. В остальном
нумерация отражает порядок приоритета: чем меньше
номер, тем важнее делать (см. раздел
...
| 14 | Кросс-сезонный прогноз состава на первый тур | 6, 7, 8, 11 | `P1` | `DONE` |
| 15 | Фиксация игроков и подбор оптимального состава под схему | 8, 9, 13 | `P2` | `DONE` |
| 16 | Учёт соперников следующего тура в оптимизации | 8, 14 | `P2` | `DONE` |
| 17 | Ручное обновление из frontend | 5, 10 | `P2` | `DONE` |
| 18 | Сила команд и AI-коэффициенты | 6, 7, 14 | `P3` | `READY` |
| 19 | Backtesting и сравнение моделей | 2, 7 | `P3` | `DONE` |
| 20 | Интеграционная сборка и эксплуатационная готовность | 9, 17, 19 | `P3` | `READY` |
| 21 | Замены на тур, карточка игрока по ховеру и прошлый сезон | 7, 8, 9, 13, 14 | `P1` | `DONE` |
| 22 | Калибровка прогноза: меньше прошлого сезона, усадка ставок, порог выгоды замены | 6, 7, 8, 14, 19 | `P1` | `DONE` |
...
flowchart LR
    ...
    S17 --> S20
    S19 --> S20
    S7 --> S21[21 Transfers UX & prior season]
    S8 --> S21
    S9 --> S21
    S13 --> S21
    S14 --> S21
...
- **Шаг 14 (кросс-сезонный прогноз).** Прямо реализует основную задачу — состав
  на 1-й тур из прошлогодних данных с учётом ушедших и новых игроков. `DONE`.
- **Шаг 21 (замены на тур, карточка по ховеру, прошлый сезон).** Закрывает то, из
  чего складывается ежедневная работа менеджера: понятный план замен, полные
  числа по игроку без перехода на другой экран и заполненная колонка «Прогноз».
  `DONE`.
- **Шаг 22 (калибровка прогноза).** Прогноз на ближайший тур переставал
  доверять текущему сезону: прошлый сезон входил в ставки всей длиной, ставки
  по двум-трём матчам не усаживались, новички получали шанс сыграть 1 к 2, а
  план замен тратил замену ради +0.05 очка. Исправлено и проверено бэктестом
  на полных сезонах 2025/26 обеих лиг с настоящим прошлым сезоном. `DONE`.
- **Шаг 24 (еврокубки).** Фэнтези Лиги чемпионов стартует без единого
  сыгранного матча, а её клубы уже играют свои чемпионаты. Прогноз еврокубка
  собирается из параллельных сезонов национальных лиг по общим стат-слагам
  игроков и клубов, с коэффициентами лиг и котировками сверху
  ([`european-cups-plan.md`](european-cups-plan.md)). `DONE`.
- **Шаг 23 (план улучшений прогноза).** Восемнадцать пунктов
  [`forecast-improvement-plan.md`](forecast-improvement-plan.md): найденное в
  остатках третье очко за полные 90 минут и редкие события в таблице
  начисления, спад внутри сезона, домашнее поле множителем, переходы внутри
  сезона, длина дисквалификации, метрики ранжирования и мультилиговый бэктест,
  капитан по верхнему хвосту, замены на горизонт двух туров, скамейка под
  автозамены, обученная модель-претендент, история котировок, импорт сезонов
  с нечитаемым игроком, ещё по сезону на лигу. `DONE`.

### P2 — усиление подбора состава

- **Шаг 15 (фиксация игроков).** Явно запрошенная функция, повышает практическую
  пользу подбора. `DONE`.
- **Шаг 16 (учёт соперников).** Улучшает оптимизацию на очных встречах. `DONE`.
- **Шаг 17 (ручное обновление из frontend).** Операционное удобство; не блокирует
  цель (обновление доступно через админ-API/CLI). `DONE`.
...
- **Шаг 18 (сила команд и AI-коэффициенты).** Наибольшая сложность (discovery по
  ...
  Бэктест шага 19 даёт измеримую точку отсчёта: вердикт `revise_model` (событийная
  модель точнее на игравших и собирает состав лучше, но проигрывает baseline на
  полной выборке), поэтому эффект новых коэффициентов проверяется тем же прогоном.
  `READY`.
- **Шаг 19 (backtesting).** Внутренняя валидация модели; ценен для доверия к
  прогнозу, но не блокирует сбор состава. `DONE`.
- **Шаг 20 (интеграционная готовность).** Деплой уже работает; остаётся операционка
  (graceful shutdown, structured logs, smoke-test). Зависимости 17 и 19 завершены.
  `READY`.
...
Завершённые шаги (0–17, 19 и 21) даны сводкой «цель + зависимости»; их детали и
карточки выполнения — в [`docs/development-history.md`](development-history.md).
Незавершённые (18 и 20) описаны полностью (детали, критерии приёмки, границы).
...
- **Шаг 14. Кросс-сезонный прогноз состава на первый тур** — зависимости: 6, 7,
  8, 11. Прогноз и состав на 1-й тур нового турнира из данных прошлого сезона по
  кросс-сезонной идентичности (`stat_player_id`/`stat_team_id`): ушедшие игроки
  исключаются, новички получают позиционные прайоры (`is_newcomer`), каждая
  строка помечена источником (`stat_source`), фронт визуально разделяет сезоны
  (запросы 1 и 2).
- **Шаг 15. Фиксация игроков и подбор оптимального состава под схему** —
  зависимости: 8, 9, 13. Закрепить игроков, которых пользователь точно хочет
  видеть в составе (`locked_ids`/`locked_starter_ids`), задать схему стартовой
  одиннадцатки (`formation`) и добрать остальных оптимально; несовместимый набор
  фиксаций отклоняется с указанием конфликтующего правила (запрос 7).
- **Шаг 16. Учёт соперников следующего тура в оптимизации** — зависимости: 8, 14.
  Календарь тура в целевой функции: пара стартеров-соперников штрафуется на
  модуль взаимного гашения их прогнозов («сухарь» обороны против голов атаки), так
  что берётся только при чистой выгоде по ожидаемым очкам; эффект объясняется в
  отчёте (`solution.fixtures`) и перепроверяется независимым валидатором
  (запрос 8).
- **Шаг 17. Ручное обновление из frontend** — зависимости: 5, 10. Безопасная
  кнопка обновления перед туром: экран `/admin` запускает реальный ingestion job,
  показывает этап (стадии пишутся воркером в `ingestion_jobs`), прогресс, время
  запуска и длительность, опрашивает статус только пока job активен, блокирует
  параллельный запуск, после успеха показывает новый снапшот и целевой тур, а
  после ошибки — безопасное сообщение и повтор.
- **Шаг 19. Backtesting и сравнение моделей** — зависимости: 2, 7.
  Walk-forward прогон сезона по турам (`fantasy-backtest`): точность MAE/RMSE/bias
  по турам и позициям, реальные очки собранных составов против hindsight-оптимума,
  сравнение с двумя baseline, независимый аудит cutoff (пересчёт истории из
  сырой таблицы появлений) и явный вердикт `accept_model` / `revise_model` /
  `keep_baseline`.
- **Шаг 21. Замены на тур, карточка игрока по ховеру и прошлый сезон** —
  зависимости: 7, 8, 9, 13, 14. Сделать ежедневный цикл менеджера
  самодостаточным: план замен называет обе стороны обмена («кого на кого») в
  пределах лимита тура и не предлагает бесполезных замен; карточка игрока
  открывается по ховеру на поле, в пуле и в таблице; прошлый сезон доступен и в
  API, и в UI; колонка «Прогноз» заполняется без ручного запуска
  `fantasy-forecast`; три кнопки оптимизатора объясняют, чем они отличаются
  (запросы 1–5 по ревью развёрнутого приложения).
- **Шаг 22. Калибровка прогноза: меньше прошлого сезона, усадка ставок, порог
  выгоды замены** — зависимости: 6, 7, 8, 14, 19. По запросу ревью «прогноз
  выдаёт игроков, которые в текущем сезоне набирают сильно меньше»: прошлый
  сезон входит в per-90 ставки не всей длиной, а окном `RATE_PRIOR_WINDOW`
  матчей с полураспадом 3 матча; ставки усаживаются к среднему по позиции
  (`RATE_SHRINK_MATCHES`); блочные награды считаются условно на выход на поле,
  а штраф за пропущенные — по доле сыгранных минут; вероятность выхода новичка
  зависит от цены и быстро гаснет после пропущенного матча; клубные силы
  усаживаются к среднему по лиге; более поздний сезон никогда не берётся как
  «прошлый»; план замен предлагает замену только при выгоде не меньше
  `min_transfer_gain` (0.5 очка). Карточка выполнения — в
  [`docs/development-history.md`](development-history.md).
- **Шаг 23. Выполнение плана улучшений прогноза** — зависимости: 22. Все
  пункты [`forecast-improvement-plan.md`](forecast-improvement-plan.md)
  (статус каждого — в его конце): таблица начисления `rpl-2025-2026.3`
  (полные 90 минут, красные, автоголы, пенальти — 99.6–100 % точного
  восстановления), features `1.7.0` (спад ставок внутри сезона 0.95, усадка 8,
  пул сил клубов по площадкам, переходы внутри сезона, второй матч после
  прямой красной, даты окончания дисквалификаций), бэктест `1.1.0` (ранговая
  корреляция, топ-25, автозамены, перенос состава между турами, несколько лиг
  одной командой), оптимизатор `1.5.0` (капитан по верхнему хвосту 0.5σ,
  горизонт 2 тура, порог по неопределённости, скамейка под автозамены),
  обученная модель `ridge_stack`, `match_odds_history`, пропуск нечитаемого
  игрока при импорте. Карточка выполнения — в
  [`docs/development-history.md`](development-history.md).
- **Шаг 24. Еврокубки из национальных чемпионатов** — зависимости: 22, 23.
  План — [`european-cups-plan.md`](european-cups-plan.md). Матчинг игроков и
  клубов между турнирами уже даёт схема (глобальные стат-слаги Sports.ru);
  features `1.8.0` читает для Лиги чемпионов и Лиги Европы параллельные
  сезоны национальных лиг как слои истории (`PARALLEL_WEIGHT`, коэффициенты
  лиг `LEAGUE_STRENGTH`, отсечка по дедлайну тура, травмы из снапшота лиги,
  красные карточки только внутри турнира, метка `parallel_league`, список
  `sources`), сила клубов из чемпионата, `fantasy-backtest --no-parallel` как
  базовая линия, импорт ЛЧ 26/27 и 25/26 и семи лиг за три сезона. Карточка
...
Проверка эффекта: прогон `fantasy-backtest` (шаг 19) до и после изменения. Точка
отсчёта на сезоне 2025/2026 — `docs/backtest-2025-2026.md`: `poisson_events`
MAE 1.0899 (на игравших 2.0962), очки составов 1690 против 1667 у `season_mean`
при hindsight-оптимуме 3753, вердикт `revise_model`. Наиболее нестабильные
признаки в том же отчёте — как раз `opponent_attack`/`opponent_defense` и
`club_attack`/`club_defense`, то есть текущая грубая оценка силы команд.
...
Ещё не сделано в рамках полного шага 20 (поэтому статус — `READY`): graceful
shutdown и structured logs, проверка совместимости схемы при деплое и smoke-test
первой версии в README. Автодеплой-часть и развёртывание на сервере выполнены,
кнопка обновления данных из UI (шаг 17) и backtesting (шаг 19) — завершены.
